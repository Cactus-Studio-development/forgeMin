...
export interface AEUser {
  id: string; // userId matches Firebase Auth UID
  app: string; // 'argentinaEmpleos' - Identificador de ruta/módulo de app
  appType: string; // 'argentinaEmpleos'
  userType: AEUserType; // Perfil: candidato, empresa, reclutador, ambos
  name: string;
  email: string;
  photoUrl?: string;
  age?: number;
...
export interface AEWithdrawal {
  ...
  destinationAccount: string; // CVU / Alias / Email
...
export interface AEAdminLog {
  ...
  action: string;
  targetId: string;
  targetType: 'user' | 'job' | 'wallet' | 'withdrawal' | 'ai_generation';
...
export interface AEProvince {
  id: string;
  name: string;
}

export interface AECity {
  id: string;
  provinceId: string;
  name: string;
}
